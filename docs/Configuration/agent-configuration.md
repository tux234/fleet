...
- `options` include the agent settings listed under `osqueryOptions` [in the agent options source code](https://github.com/fleetdm/fleet/blob/main/server/fleet/agent_options_generated.go). These can be updated without a fleetd restart.
- `command_line_flags` include the agent settings listed under osqueryCommandLineFlags [in the agent options source code](https://github.com/fleetdm/fleet/blob/main/server/fleet/agent_options_generated.go). These are only updated when fleetd restarts.
...
You can use Fleet to configure the `yara` and `yara_events` osquery tables. Learn more about [YARA configuration and continuous monitoring](https://fleetdm.com/guides/remote-yara-rules#basic-article).

## extensions

> This feature requires a custom TUF auto-update server (available in Fleet Premium). Learn more about [fleetd updates](https://fleetdm.com/guides/fleetd-updates).
